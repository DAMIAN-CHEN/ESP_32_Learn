...
#define CONFIG_TM1637_BRIGHTNESS 0x04
...
static void tm1637_delay(uint32_t us);
static void tm1637_ack(tm1637_led_t * led);
...
void tm1637_ack(tm1637_led_t * led)
{
    // Send start signal
    // Both outputs are expected to be HIGH beforehand
    gpio_set_level(led->m_pin_clk, 0);
    ...
    while(gpio_get_level(led->m_pin_dta));
    gpio_set_level(led->m_pin_clk, 1);
    tm1637_delay(2);
